...
public class RebuildDownloadHistoryTask : IScheduledTask
{
    ...
    /// <inheritdoc />
    public string Key => "AniBridgeRebuildDownloadHistory";
    ...
    /// <inheritdoc />
    public string Category => "AniBridge Downloader";
    ...
    private static List<(string BasePath, string Language, string Source)> CollectScanTargets(PluginConfiguration config)
    {
        ...
        // Every registered site's per-language and general download paths
        foreach (var entry in config.Sites)
        {
            foreach (var (langKey, path) in entry.Config.DownloadPaths)
            {
                AddTarget(path, langKey, entry.Source);
            }

            if (!string.IsNullOrEmpty(entry.Config.DownloadPath))
            {
                AddTarget(entry.Config.DownloadPath, config.GetPreferredLanguage(entry.Source), entry.Source);
            }
        }
