...
/// <summary>
/// Service for interacting with the s.to (SerienStream) website. English Dub only (s.to does
/// not offer an English sub track); German tracks are intentionally excluded from
/// <see cref="LanguageMap"/>. s.to uses a completely different HTML template than aniworld.to,
/// so most extraction methods are overridden here.
/// </summary>
public class StoService : HtmlScrapingSiteService
{
    private static readonly IReadOnlyDictionary<string, string> StoLanguageMap =
        new Dictionary<string, string> { ["2"] = "dub" };

    private static readonly string[] StoLegacyHosts =
    {
        "s.to", "www.s.to", "serienstream.to", "www.serienstream.to",
    };
    ...
    /// <inheritdoc />
    public override string DisplayName => "s.to";
    ...
    public override string BaseUrl
    {
        get
        {
            var custom = Plugin.Instance?.Configuration?.GetSiteConfig("sto")?.CustomBaseUrl;
            return string.IsNullOrWhiteSpace(custom) ? "https://s.to" : custom.TrimEnd('/');
        }
    }

    /// <summary>
    /// s.to's native language keys: 1 = German Dub, 2 = English Dub. Only English Dub ("2")
    /// is mapped through — German is dropped.
    /// </summary>
    protected override IReadOnlyDictionary<string, string> LanguageMap => StoLanguageMap;

    /// <inheritdoc />
    public override bool AllowInsecureHttp =>
        Uri.TryCreate(BaseUrl, UriKind.Absolute, out var uri) && uri.Scheme == Uri.UriSchemeHttp;

    /// <inheritdoc />
    public override IEnumerable<string> AllowedHosts => base.AllowedHosts.Concat(StoLegacyHosts);
