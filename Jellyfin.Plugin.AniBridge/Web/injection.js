(function () {
    ...
    function injectSidebarEntry() {
        ...
        entry.innerHTML = '<span class="material-icons navMenuOptionIcon download" aria-hidden="true"></span>' +
            '<span class="navMenuOptionText">AniBridge Downloader</span>';
    ...
    function showModal() {
        ...
        title.textContent = 'AniBridge Downloader';
    ...
    function loadPage(content) {
        ...
        api.fetch({
            ...
            var scriptUrl = api.getUrl('AniBridge/PageScript') + '?_t=' + Date.now();
            import(scriptUrl).then(function (module) {
                ...
                console.error('AniBridge: Failed to load page script:', err);
            });
        }).catch(function (err) {
            content.innerHTML = '<div style="padding:2em;text-align:center;opacity:0.6;">' +
                'Failed to load AniBridge Downloader. Please check your permissions.</div>';
            console.error('AniBridge: Failed to load page:', err);
        });
    }
